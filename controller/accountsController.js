  ...
  try {
    ...
    if (!account) {
      ...
      return res.status(404).json({
        message: "Account not found for the provided Account ID",
      })
    }
...
function validateAccountStructure(accountGroup, accountHead, accountSubhead) {
  return validStructure[accountGroup]?.[accountHead]?.includes(accountSubhead) || false
}
...
function validateAccountStructure(accountGroup, accountHead, accountSubhead, errors) {
  validateField(
    !validStructure[accountGroup]?.[accountHead]?.includes(accountSubhead) || false,
    "Invalid Account Group, Head, or Subhead.",
    errors
  );
}
...
function validateAlphabetsFields(fields, data, errors) {
  fields.forEach((field) => {
    if (data[field] !== undefined) {
      validateField(!isAlphabets(data[field]), field.charAt(0).toUpperCase() + field.slice(1) + " should contain only alphabets.", errors)
    }
  })
}
