...
function NewJournal({ page }: Props) {
  ...
  const { request: EditJournal } = useApi("put", 4000);
  // const { request: GetLastJournelPrefix } = useApi("get", 4000);
  const { request: GetAllAcounts } = useApi("get", 4000);
  ...
  const currencies = [
    { value: "AED", display: "AED - Arab Emirates dirham" },
  ...
  const handleAddNewJournel = async () => {
    ...
    if (errors.length > 0) {
      ...
      try {
        const url =
          page === "edit"
            ? `${endpoints.EDIT_JOURNAL_ENTRY}/${id}`
            : `${endpoints.ADD_NEW_JOURNEL_ENTRY}`;
                ...
                {newJournalDatas.transaction.map((row, index) => (
                  <tr className="border-b border-tableBorder" key={index}>
                    <AccountDropdown
                      ...
                      isDropdownOpen={isAccountDropdownOpen[index] || false}
