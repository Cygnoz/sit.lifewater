...
const AddUnloadStock: React.FC = () => {
  ...
  console.log("Unload stock details : ", orderDetails);
  ...
  const [selectedSubRoute, setSelectedSubRoute] = useState<any>();
...
console.log(mainRouteId);
  ...
  const handleSubRouteChange = (
    ...
    if (!selectedSubRoute) {
      // Handle case when no sub-route is selected (e.g., if data doesn't match)
      console.error(`SubRoute not found: ${subRouteName}`);
      return;
    }

    const subRouteId = selectedSubRoute._id;
    const AllStock = selectedSubRoute.stock || [];

    // Update state
    setSelectedSubRoute(subRouteName);
    setFilteredItems(AllStock); // Set filtered items based on the selected sub-route

    // Update order details with selected subRouteName and subRouteId
  ...
  const { request: AddUnloadStock } = useApi("post", 4001);
  const navigate = useNavigate();

  const handleSubmit = async () => {
    try {
      const url = `${endpoints.ADD_UNLOAD_STOCK}`;
      const { response, error } = await AddUnloadStock(url, orderDetails);
      if (!error && response) {
