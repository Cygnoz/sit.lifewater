...
const CreditCollection: React.FC = () => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (name === "customerId") {
      const customerOrders = orders.filter(order => order.customerId === value && order.paymentMode === "Cash" && order.balanceAmount > 0);
  ...
  return (
    <div className="flex justify-center items-start min-h-screen bg-gray-100 p-5">
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow-md rounded-lg p-6 w-full max-w-md space-y-4">
        ...
        <label className="block text-sm font-medium text-gray-700">Collected Amount</label>
        <input type="number" name="paidAmount" value={formData.paidAmount} onChange={handleChange} className="w-full p-2 border rounded-md" />
        ...
        <div className="flex justify-between mt-5">
          <Link to="/collection" className="py-1 px-8 bg-gray-100 text-red-900 border border-red-900 rounded-lg hover:bg-gray-300">
            Cancel
          </Link>
